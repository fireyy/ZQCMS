<?php
class gameinfo_controller {
    public function __construct() {
	$this->model = zq_core::load_model("game_info_model");
	...
	if($data){
	    ...
	    for ($c = 0;$c < $count;++$c){
	        ...
	        if($guid){
                    switch ($method) {
                        case "add":
	        	    if ($this->model->addGameInfo($_data)){
	        	    }else{
	        		$errorids[] = $guid;
	        		$status = -2;
	        		$msg = "插入数据失败.";
                	    ...
                	    break;
                        case "delete":
                	    $this->model->deleteGameinfo($guid);
                	    break;
                        case "update":
                	    $this->model->updateGameInfo($_data);
                	    break;
                        default:
